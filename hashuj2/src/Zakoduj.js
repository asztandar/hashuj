...
async function handleClick(input, checkbox) {
    const checkExist = await fetch("http://hashuj-labproj24.apps.ocp.lab.cloudpak.site:3002/existModule")
...
async function encrypt(input,checkbox) {
    ...
    const odp = await fetch("http://hashuj-labproj24.apps.ocp.lab.cloudpak.site:3002/encryptText", requestOptions)
        .then((response) => response.json())
        .then((data) => {
            return data;
        })
